...
{
  "intent": "answering" | "more_cards" | "filter_change" | "off_topic" | "compare" | "done",
...
- filter_change: wants to narrow or change criteria ("any cheaper?", "IIM options?", "only online", "something shorter"). Set filter_override.
...
_VALID_INTENTS = {"answering", "more_cards", "filter_change", "off_topic", "compare", "done"}
...
def classify_intent(
    ...
    """Classify a post-recommendation message. Returns {"intent": str, "filter_override": dict|None}."""
    client = client or OpenAI()
    model = os.getenv("OPENAI_MODEL_EXTRACT", "gpt-4o-mini")
    ...
    try:
        ...
        return {"intent": "answering", "filter_override": None}
    ...
    if not isinstance(override, dict):
        override = None
    return {"intent": intent, "filter_override": override}
